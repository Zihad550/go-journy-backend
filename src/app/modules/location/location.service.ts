...
const MAPBOX_DIRECTIONS_API_URL = process.env.MAPBOX_DIRECTIONS_API_URL || 'https://api.mapbox.com/directions/v5';
const MAPBOX_GEOCODING_API_URL = process.env.MAPBOX_GEOCODING_API_URL || 'https://api.mapbox.com/geocoding/v5';
...
const updateDriverLocation = async (
  ...
  if (activeRide) {
    ...
    try {
      ...
      socketService.broadcastDriverLocation(activeRide._id.toString(), driver._id.toString(), location);
    } catch (error) {
...
const getDriverLocation = async (
  ...
  const driverLocation = await DriverLocation.findOne({ driverId: driverObjectId });
...
const reverseGeocode = async (lat: number, lng: number): Promise<IReverseGeocodingResult> => {
  ...
  try {
    ...
    return {
      ...
      address: {
        ...
        street: feature.properties?.address,
        city: feature.context?.find((c: any) => c.id.includes('place'))?.text,
        district: feature.context?.find((c: any) => c.id.includes('region'))?.text,
        country: feature.context?.find((c: any) => c.id.includes('country'))?.text,
        postcode: feature.properties?.postcode,
      },
    };
